...
module overmind::NonFungibleToken {
    ...
    */
    public fun mint_nft(
        ...
        let paymentCoinVal = coin::value(payment_coin);
        let requiredCoinVal = (1000000000 as u64);
        assert!(!(paymentCoinVal < requiredCoinVal), EInsufficientPayment);
        let currentPaymentBalance = coin::balance_mut(payment_coin);
        let res = coin::take(currentPaymentBalance, requiredCoinVal, ctx);
        let minterCapBalance = &mut minter_cap.sales;
        coin::put(minterCapBalance,res);

        let nftId = object::new(ctx);
        event::emit(NonFungibleTokenMinted {
            nft_id: object::uid_to_inner(&nftId),
            recipient,
        });
    ...
    */
    public fun combine_nfts(
        ...
        event::emit(NonFungibleTokenCombined {
            ...
            new_nft_id: object::uid_to_inner(&nftId),
        });
        let nameStr = &mut name(&nft1);
        string::append_utf8(nameStr, b" + ");
        string::append(nameStr, name(&nft2));
        let descriptionStr = &mut string::utf8(b"Combined NFT of ");
        string::append(descriptionStr, name(&nft1));
        string::append_utf8(descriptionStr, b" and ");
        string::append(descriptionStr, name(&nft2));

        let NonFungibleToken { id: id1, name: _, description: _, image: _ } = nft1;
        let NonFungibleToken { id: id2, name: _, description: _, image: _ } = nft2;
    ...
    */
    public fun withdraw_sales(
        ...
        let takenBalance = balance::withdraw_all(&mut minter_cap.sales);
        event::emit(SalesWithdrawn {
            amount: balance::value(&takenBalance),
        });
        coin::from_balance(takenBalance, ctx)
    }
